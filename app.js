...
function tickClock(){
  ...
  const dfmt = new Intl.DateTimeFormat('fr-FR', {weekday:'long', year:'numeric', month:'long', day:'numeric', timeZone:'Europe/Paris'});
...
['input','change'].forEach(ev => timerInput.addEventListener(ev, setTimerFromInput));

function startTimer(){
  timerAlert.hidden = true;
  const ms = toMS(timerInput.value.trim());
  if (ms == null) { timerInput.focus(); return; }
...
const inc = (m=0,s=0)=>{
  ...
  const mm = Math.floor(total/60); const ss = Math.floor(total%60);
...
function updateChrono(){
  const elapsed = chronoAccum + (chronoRunning ? (performance.now() - chronoStart) : 0);
...
chronoLap.addEventListener('click', () => {
  ...
  li.innerHTML = `<span>Tour</span><span class="badge">${fromMSmmsscc(elapsed)}</span>`;
  laps.prepend(li);
});
...
function saveAlarms(){ localStorage.setItem('oclock-alarms', JSON.stringify(alarms)); }

function hhmmToNextDate(hhmm){
  const [h,m] = hhmm.split(':').map(x=>parseInt(x,10));
  const now = new Date();
  const tz = 'Europe/Paris';
  const nowInTz = new Date(new Intl.DateTimeFormat('en-US', { timeZone: tz, hour12:false, year:'numeric', month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit', second:'2-digit'}).format(now));
...
function renderAlarms(){
  ...
  for (const a of alarms){
    ...
    const status = pastToday ? 'passée (rejouera demain)' : `dans ${humanize(msDiff)}`;
    const li = document.createElement('li');
    li.innerHTML = `
      <div>
      ...
      <div class="actions">
        ...
        <button data-act="toggle">${a.enabled ? 'Désactiver' : 'Activer'}</button>
...
alarmForm.addEventListener('submit', (e) => {
  ...
  const a = { id: crypto.randomUUID(), time: alarmTime.value, message: alarmMsg.value.trim(), enabled: true };
  alarms.push(a);
  saveAlarms();
  renderAlarms();
  alarmForm.reset();
...
alarmList.addEventListener('click', (e) => {
  ...
  if (act==='delete'){ alarms = alarms.filter(x=>x.id!==id); }
...
function pollAlarms(){
  const now = new Date();
  const tzNow = new Intl.DateTimeFormat('fr-FR', {timeZone:'Europe/Paris', hour:'2-digit', minute:'2-digit', hour12:false}).format(now);
  for (const a of alarms){
    if (!a.enabled) continue;
    if (tzNow === a.time){
      // Trigger
      a.enabled = false; // prevent repeat in same minute
      saveAlarms(); renderAlarms();
